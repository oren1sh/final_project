	...
	def predictByModel(self,modelPath="Models\\RotTrainedNetModel.h5"):
		...
		for dirpath, dirnames, filenames in os.walk("InputData"):
			for filename1 in [f for f in filenames if f.endswith(".png") or f.endswith(".jpg") or f.endswith(".jpeg")]:
				...
				image = FacesHelper.load_image_file(filename)#load the image
				face_locations = FacesHelper.face_locations(image,number_of_times_to_upsample = 0, model="cnn")#detect face locations using CNN
				landmarks = FacesHelper.face_landmarks(image,face_locations)#detect the landmarks of the face
				face_points = []
				...
				if len(face_points) < 68:#if face is not detected, continue
					...
					continue
				features = FacesHelper.compute_features(face_points = face_points)#compute euclidian distances between the points
				...
				#rx,ry,rz,tx,ty,tz = y_pred[0]
				rx,ry,rz = y_pred[0]
				#csvHelper.addRow(i,newFileName,rx,ry,rz,tx,ty,tz)
				csvHelper.addRow(i,newFileName,rx,ry,rz,"tx","ty","tz")
				i = i + 1
